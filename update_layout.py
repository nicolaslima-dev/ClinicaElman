    ...
    <nav class="lg:hidden glass-sidebar relative z-40 border-b border-white/10 shrink-0">
        ...
        <el-disclosure id="mobile-menu" hidden class="block lg:hidden transition-all duration-300 ease-out data-closed:opacity-0 data-closed:-translate-y-2 origin-top relative z-50">
            <div class="space-y-1 px-4 pt-2 pb-4 bg-ink-900/95 backdrop-blur-md border-t border-white/10 shadow-premium absolute w-full left-0">
                <a href="index.html" class="block rounded-md {dash_class} px-3 py-2.5 text-base font-medium transition-all">Dashboard</a>
                <a href="atendimento.html" class="block rounded-md {aten_class} px-3 py-2.5 text-base font-medium transition-all">Atendimento</a>
                <a href="faturamento.html" class="block rounded-md {fatu_class} px-3 py-2.5 text-base font-medium transition-all">Faturamento TISS</a>
                <a href="auditoria.html" class="block rounded-md {audi_class} px-3 py-2.5 text-base font-medium transition-all flex justify-between items-center">
                    Auditoria
...
for file in html_files:
    ...
    # 1. Update body class and inject tailwindplus script
    if '<body class="text-ink-800 flex h-screen overflow-hidden">' in content:
        content = content.replace('<body class="text-ink-800 flex h-screen overflow-hidden">',
    ...
    elif '<body class="text-ink-800 flex flex-col h-screen overflow-hidden bg-[#f5f6f5]">' in content:
        content = content.replace('<body class="text-ink-800 flex flex-col h-screen overflow-hidden bg-[#f5f6f5]">',
                                  '<body class="text-ink-800 flex flex-col lg:flex-row h-screen overflow-hidden bg-[#f5f6f5]">\n    <!-- @tailwindplus/elements for Interactive Navbar -->\n    <script src="https://cdn.jsdelivr.net/npm/@tailwindplus/elements@1" type="module"></script>')
                                  
    # 2. Update aside class
    if 'class="w-72 glass-sidebar flex flex-col' in content:
        content = content.replace('class="w-72 glass-sidebar flex flex-col', 'class="w-72 glass-sidebar hidden lg:flex flex-col')
        
    # 3. Inject Mobile Navbar before <main
    if '<!-- MOBILE TOP NAVIGATION -->' not in content:
        dash_class = "bg-white/10 text-white" if file == 'index.html' else "text-brand-100/50 hover:bg-white/5 hover:text-white"
        aten_class = "bg-white/10 text-white" if file in ['atendimento.html', 'novo_atendimento.html'] else "text-brand-100/50 hover:bg-white/5 hover:text-white"
        fatu_class = "bg-white/10 text-white" if file == 'faturamento.html' else "text-brand-100/50 hover:bg-white/5 hover:text-white"
        audi_class = "bg-white/10 text-white" if file == 'auditoria.html' else "text-brand-100/50 hover:bg-white/5 hover:text-white"
        
        navbar = MOBILE_NAVBAR_TEMPLATE.format(
            dash_class=dash_class,
            aten_class=aten_class,
            fatu_class=fatu_class,
            audi_class=audi_class
        )
        
        content = content.replace('    </aside>\n\n    <main', f'    </aside>\n{navbar}\n    <main')
        content = content.replace('    </aside>\n    <main', f'    </aside>\n{navbar}\n    <main') # Fallback if single newline
        
    # 4. Update header class
    if 'class="h-20 px-8 flex items-center justify-end' in content:
        content = content.replace('class="h-20 px-8 flex items-center justify-end', 'class="h-20 px-8 hidden lg:flex items-center justify-end')
        
    # 5. Update views-container padding
    if 'class="flex-1 overflow-y-auto p-8 relative z-10"' in content:
        content = content.replace('class="flex-1 overflow-y-auto p-8 relative z-10"', 'class="flex-1 overflow-y-auto p-4 sm:p-8 relative z-10 w-full"')
    elif 'class="flex-1 overflow-y-auto relative p-8 z-10"' in content: 
         content = content.replace('class="flex-1 overflow-y-auto relative p-8 z-10"', 'class="flex-1 overflow-y-auto relative p-4 sm:p-8 z-10 w-full"')

    with open(file, 'w', encoding='utf-8') as f:
        f.write(content)

print("All layout changes applied!")
